...
void ColorLED(void)
{
	...
	static uint8_t r_temp;
	uint8_t buf[3];
	
	short tempData,accData[3],gyroData[3];;
	float Temperature;
		...
		{
			r_temp = AT24C02_ReadOneByte(MPU6050_RA_WHO_AM_I);
			printf("Mpu6050_addr = %d",r_temp);
		}
		else if(count_temp>8)
		{
			count_temp = 10;
		...
		if(r_temp==0x68)
		{
		Mpu6050_ReadLenByte(MPU6050_ACC_OUT,2,buf);
		accData[0] = (buf[0] << 8) | buf[1];
		Mpu6050_ReadLenByte(MPU6050_ACC_OUT+2,2,buf);
		accData[1] = (buf[0] << 8) | buf[1];
		Mpu6050_ReadLenByte(MPU6050_ACC_OUT+4,2,buf);
		accData[2] = (buf[0] << 8) | buf[1];

		printf("acc_X=%8d",accData[0]);
		printf("acc_Y=%8d",accData[1]);
		printf("acc_Z=%8d\r\n",accData[2]);

		Mpu6050_ReadLenByte(MPU6050_GYRO_OUT,2,buf);
		gyroData[0] = (buf[0] << 8) | buf[1];
		Mpu6050_ReadLenByte(MPU6050_GYRO_OUT+2,2,buf);
		gyroData[1] = (buf[0] << 8) | buf[1];
		Mpu6050_ReadLenByte(MPU6050_GYRO_OUT+4,2,buf);
		gyroData[2] = (buf[0] << 8) | buf[1];
		printf("Gyro_X=%8d",gyroData[0]);
		printf("Gyro_Y=%8d",gyroData[1]);
		printf("Gyro_Z=%8d\r\n",gyroData[2]);

		Mpu6050_ReadLenByte(MPU6050_RA_TEMP_OUT_H,2,buf);
		tempData = (buf[0] << 8) | buf[1];
		Temperature=((double) (tempData /340.0))+36.53;
		printf("temp=%8.2f\r\n",Temperature);
		}
} 
